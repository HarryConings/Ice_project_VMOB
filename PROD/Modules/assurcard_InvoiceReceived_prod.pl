...
#{ # sequence of InvoiceFeedback
#
#  # is an unnamed complex
#  # occurs 1 <= # <= unbounded times
#  InvoiceFeedback =>
#  [ { # sequence of InvoiceStatus, Justification, PaymentDate,
#      #   PaymentAmount, FinancialAccount, PaymentReference
#
#      # is a xs:string
#      InvoiceStatus => "example",
#
#      # is an unnamed complex
#      # Justification is simple value with attributes
#      Justification =>
#      { # is a xs:string
#        Code => "example",
#
...
#
#      # is a xs:string
#      # attribute InvoiceId is required
#      InvoiceId => "example",
#
#      # is a xs:string
#      # attribute CardId is required
#      CardId => "example",
#
#      # is a xs:string
#      ErrorCode => "example", }, ],
#
#  # is a xs:string
#  # attribute InsurerIdentifier is required
#  InsurerIdentifier => "example", }
use strict;
use Data::Dumper;
...
&load_assurcard_setting_feedback('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\assurcard_card_generation_settings.xml');
#&ask_approved_via_webserv_client_feedback;
$mail_feedback = $mail_feedback."\nWE STUREN TERUG AAN ASSURCARD WELKE FACTUREN WE ONTVANGEN HEBBEN\n";
$mail_feedback = $mail_feedback."----------------------------------------------------------------------\n";
print "\nWE STUREN TERUG AAN ASSURCARD WELKE FACTUREN WE ONTVANGEN HEBBEN\n";
print "----------------------------------------------------------------------\n";
&ask_feedback_via_webserv_client_feedback;
&zet_feedback_to_yes_agresso ;
...
sub ask_feedback_via_webserv_client_feedback {
     ...
     my $xml_file = "$directory\\InvoiceReceived.$insurer_id.$vandaag.$tijd.xml";
     use SOAP::Lite ;
    ...
    my $proxy = 'http://S200WP1XXL01.mutworld.be/BusinessWorld-webservices/service.svc?QueryEngineService/QueryEngineV201101'; #productie    
     ...
     #my $template    = SOAP::Data->name('query:TemplateId' => "4249")->type('');
     my $template    = SOAP::Data->name('query:TemplateId' => "4446")->type('');
     ...
     if ($@) {
         print "geen nieuwe facturen\n";#code
         $mail_feedback = $mail_feedback."geen nieuwe facturen\n";
         &mail_bericht_feedback;
         die;
     }
     ...
     my $xsd = 'P:\OGV\ASSURCARD_PROG\asurcard_xsd\invoiceFeedbacks-har.xsd';
     ...
     &copy_file_to_assurcard_upload_feedback($xml_file);
    }
...
sub copy_file_to_assurcard_upload_feedback  {
     my $file = shift @_;
...
sub mail_bericht_feedback {
     ...
     foreach my $geadresseerde (@aan_lijst) {
         ...
         $smtp->datasend("\n");
         $smtp->datasend("Subject: Invoice received xml generatie verslag $vandaag");
         $smtp->datasend("\n");
...
sub zet_feedback_to_yes_agresso {
     &setup_mssql_connectie;
     my $teller = 0;
     foreach my $invoice_nr (@invoice_processed) {
         print "->$invoice_nr<-\n";
         print "voor update\n";
         print "select ext_inv_ref,voucher_no,feedback,approved FROM zgt_log_assurcard WHERE ext_inv_ref =\'$invoice_nr\'\n";
         ...
         my $updatethis = $dbh_mssql ->do("UPDATE zgt_log_assurcard set feedback = 'YES' WHERE ext_inv_ref like '$invoice_nr%'");
         print "UPDATE zgt_log_assurcard set feedback = 'YES' WHERE ext_inv_ref = \'$invoice_nr\'\n";
         $teller += 1;
         print "na update\n";
         print "select ext_inv_ref,voucher_no,feedback,approved FROM zgt_log_assurcard WHERE ext_inv_ref =\'$invoice_nr\'\n";
...
sub setup_mssql_connectie {
     my $dsn_mssql = join "", (
      ...
      my $db_options = {
         PrintError => 1,
         RaiseError => 1,
         AutoCommit => 1, # werkt niet Use transactions 0
         LongReadLen =>2000,
