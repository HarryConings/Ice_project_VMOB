...
package verwerk_files;
     ...
     sub new {
        ...
        our $mail_text;
         verwerk_files->load_agresso_setting('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\agresso_settings_V2.xml',"P:\\OGV\\ASSURCARD_$test_prod\\assurcard_settings_xml\\zet_klant_in_agresso_settings_new.xml");
         print '';
         $mail_text = &delete_lock_cache($mail_text);
         $mail_text = &clean_agresso_xml($mail_text,$today_year_month);
         $mail_text = $mail_text."\nEINDE\n____________\n";
     ...
     sub load_agresso_setting  {
         my ($class,$file_name,,$file_name1) =  @_;
         $agresso_instellingen = XMLin("$file_name");
         $zet_klant_instellingen = XMLin("$file_name1");
          print "ingelezen\n";          
        }
     ...
     sub clean_agresso_xml {
          my ($mailtext,$dir_d) = @_;
          my $directory =  $zet_klant_instellingen ->{plaats_file};
          opendir(DIR,$directory);
          my @files = grep(/\.xml$/,readdir(DIR));
          my $destination_dir ='W:\OGV\ASSURCARD_PROG\asurcard_xml_archief';
