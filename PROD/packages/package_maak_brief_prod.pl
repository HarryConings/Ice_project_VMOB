...
package Webservice_pdf_to_Cataloog;
	 ...
	 if (uc $in_cataloog eq 'JA' or uc $in_cataloog eq 'YES') {
		 ...
		 $catalog_Key = $main::gkd_soorten_brieven->{ziekenfondsen}->{"zkf$zkf"}->{doc_in_naam_mapping}->{$zoek_cat_key}->{CAT} if ($zoek_cat_key ne '') ;
		 $catalog_Key = $main::PdfToAgresso_instellingen->{Catalog_key_Openoffice_brieven} if (!defined $catalog_Key or $catalog_Key eq '' );
		 my $user= $main::PdfToAgresso_instellingen->{ziekenfondsen}->{"zkf$zkf"}->{as400_user};     	     #username as400
		 my $pass=decrypt->new($main::PdfToAgresso_instellingen->{ziekenfondsen}->{"zkf$zkf"}->{as400_paswoord});              #paswoord
