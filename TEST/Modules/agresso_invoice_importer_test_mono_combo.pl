...
use Data::Dumper;
our @combo_mono_therapie = (767874,767885,797911,767922,767896,767900,767933,767944);
our $ABWTransaction;
our $assurcard ;
...
&load_agresso_setting_invoice('P:\OGV\ASSURCARD_TEST\assurcard_settings_xml\agresso_settings.xml');
...
&read_assurcard_invoice('P:\OGV\ASSURCARD_TEST\assurcard_invoices\176.invoice.out.20200124.000554.xml');
#&modulo_97_invoice ('+++130/00a0/06609+++');

#&read_assurcard_invoice('P:\assurcard\invoices\017.invoice.out.20140213.120926.xml\017.invoice.out.20140213.120926.xml');
#&read_assurcard_invoice('P:\assurcard\invoices\067.invoice.out.20140213.114623.xml\067.invoice.out.20140213.114623.xml');

&mail_bericht_invoice;
sub read_assurcard_invoice {
     my $file = shift @_;
     #$assurcard = XMLin('P:\assurcard\invoices\067.invoice.out.20140213.114623.xml\067.invoice.out.20140213.114623.xml');
     #$assurcard = XMLin('P:\assurcard\invoices\110.invoice.out.20140213.114356.xml\test-inv-agr.xml');
     ...
     #$batch_id_teller = '20190304.001127.1';
...
sub maak_eerste_transactie_voucher_invoice {
     ...
     my $TransDate= '2019-06-03';  # vandaag in YYYYMMDD "$vandaag"
     my $Description = '';
     my $OAPartRekA = '';
     my $OAPartRekB = '';
     my $PatientPart = '';
     ...
     if ($volgnr_invoice eq 'geen' and $lijn_nr eq 'geen') {
         ...
         $ApArInfoInvoiceNo_1 = $assurcard->{Invoice}->{InvoiceNumber};$ApArInfoInvoiceNo_1 +=11;
         ...
         $ApArInfoInvoiceNo_1 = $assurcard->{Invoice}[$volgnr_invoice]->{InvoiceNumber};$ApArInfoInvoiceNo_1 +=11;
         $ApArInfoInvoiceNo = "$ApArInfoInvoiceNo/$ApArInfoInvoiceNo_1";
...
sub maak_volgende_transactie_invoice {
     ...
     if ($volgnr_invoice eq 'geen' and $volgnr_Groups eq 'geen') {
         ...
         $ApArInfoInvoiceNo = $assurcard->{Invoice}->{Id};
         $ApArInfoInvoiceNo_1 = $assurcard->{Invoice}->{InvoiceNumber};$ApArInfoInvoiceNo_1 +=2;
         $ApArInfoInvoiceNo = "$ApArInfoInvoiceNo/$ApArInfoInvoiceNo_1";
         $SundryInfoBankAccount= $assurcard->{Invoice}->{FinancialAccount};
         $ApArInfoApArNo = $assurcard->{Invoice}->{MutualNumber};
             ...
             if ($@) {
                 ...
                 foreach my $volgnr_Col (keys @{$assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Col}}) {
                     ...
                     if ($assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Col}[$volgnr_Col]->{Type} eq "Type") {
                         $Description =$assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Col}[$volgnr_Col]->{content};
                       }
                     ...
                     if (!$@) {
                          ...
                          if (!$@) {
                               ...
                               my $Description_warning= $assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Warnings}->{Warning}->{Description};
                               ...
                               eval {my $bestaat2 = $assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Warnings}[0]->{Warning}};
                                if (!$@) {
                                    ...
                                    my $Description_warning= $assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Warnings}[0]->{Warning}->{Description};
                                    $Description_warning = substr ($Description_warning,0,235);
                                    $ExternalRef = "$code_warning : $Description_warning";
                                     $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                   }
                     ...
                     foreach my $volgnr_Col (keys @{$assurcard->{Invoice}->{Groups}->{Group}[$volgnr_Group]->{Line}->{Col}}) {
                         ...
                         if (!$@) {
                          ...
                          if (!$@) {
                               ...
                               my $Description_warning= $assurcard->{Invoice}->{Groups}->{Group}[$volgnr_Group]->{Line}->{Warnings}->{Warning}->{Description};
                               $Description_warning = substr ($Description_warning,0,235);
                               $ExternalRef = "$code_warning : $Description_warning";
                                $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                              }else {
                               ...
                               if (!$@) {
                                    ...
                                    my $Description_warning= $assurcard->{Invoice}->{Groups}->{Group}[$volgnr_Group]->{Line}->{Warnings}[0]->{Warning}->{Description};
                                    $Description_warning = substr ($Description_warning,0,235);
                                    $ExternalRef = "$code_warning : $Description_warning";
                                    $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                   }
                         ...
                         foreach my $volgnr_Col (keys @{$assurcard->{Invoice}->{Groups}->{Group}[$volgnr_Group]->{Line}[$volgnr_Line]->{Col}}) {
                             ...
                             if (!$@) {
                               ...
                               if (!$@) {
                                    ...
                                    my $Description_warning= $assurcard->{Invoice}->{Groups}->{Group}[$volgnr_Group]->{Line}[$volgnr_Line]->{Warnings}->{Warning}->{Description};
                                    $Description_warning = substr ($Description_warning,0,235);
                                    $ExternalRef = "$code_warning : $Description_warning";
                                    $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                   }else {
                                    ...
                                    if (!$@) {
                                         ...
                                         my $Description_warning= $assurcard->{Invoice}->{Groups}->{Group}[$volgnr_Group]->{Line}[$volgnr_Line]->{Warnings}[0]->{Warning}->{Description};
                                         $Description_warning = substr ($Description_warning,0,235);
                                         $ExternalRef = "$code_warning : $Description_warning";
                                         $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                        }
         ...
         $GLAnalysisDim3 = substr($GLAnalysisDim3,5,11);
         $ApArInfoInvoiceNo = $assurcard->{Invoice}[$volgnr_invoice]->{Id};
         $ApArInfoInvoiceNo_1 = $assurcard->{Invoice}[$volgnr_invoice]->{InvoiceNumber};$ApArInfoInvoiceNo_1 +=2;
         $ApArInfoInvoiceNo = "$ApArInfoInvoiceNo/$ApArInfoInvoiceNo_1";
         $SundryInfoBankAccount= $assurcard->{Invoice}[$volgnr_invoice]->{FinancialAccount};
         $ApArInfoApArNo = $assurcard->{Invoice}[$volgnr_invoice]->{MutualNumber};
         ...
         if ($@) {
             ...
             if ($@) {
                     ...
                     eval {my $bestaat = $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}->{Warnings}};
                             if (!$@) {
                               ...
                               if (!$@) {
                                    ...
                                    my $Description_warning= $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}->{Warnings}->{Warning}->{Description};
                                    $Description_warning = substr ($Description_warning,0,235);
                                    $ExternalRef = "$code_warning : $Description_warning";
                                    $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                    ...
                                    if (!$@) {
                                         ...
                                         my $Description_warning= $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}->{Warnings}[0]->{Warning}->{Description};
                                         $Description_warning = substr ($Description_warning,0,235);
                                         $ExternalRef = "$code_warning : $Description_warning";
                                         $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                        }
                 ...
                 undef $transactie_onderdeel;
                 #if ( $GLAnalysisDim2 ~~ @combo_mono_therapie ) {
                 #                       $GLAnalysisDim5 = 761434;
                 #                       $AmountsAmount_OAPART =0;
                 #                       $AmountsCurrAmount_OAPART=0;
                 #                  }   
                 $RoomTypeCode_alg = $GLAnalysisDim5 if ($RoomTypeCode_alg < $GLAnalysisDim5);
                 $GLAnalysisDim6  =~ s/-//g;$GLAnalysisDim6  =~ s/\s//g;
                 $GLAnalysisDim7  =~ s/-//g;$GLAnalysisDim7  =~ s/\s//g;
                         ...
                         foreach my $volgnr_Col (keys @{$assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}[$volgnr_Line]->{Col}}) {
                             ...
                             if (!$@) {
                               ...
                               if (!$@) {
                                    ...
                                    my $Description_warning= $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}[$volgnr_Line]->{Warnings}->{Warning}->{Description};
                                    $Description_warning = substr ($Description_warning,0,235);
                                    $ExternalRef = "$code_warning : $Description_warning";
                                     $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                   }else {
                                    ...
                                    if (!$@) {
                                         ...
                                         my $Description_warning= $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}[$volgnr_Line]->{Warnings}[0]->{Warning}->{Description};
                                         $Description_warning = substr ($Description_warning,0,235);
                                         $ExternalRef = "$code_warning : $Description_warning";
                                          $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                        }
                         ...
                         foreach my $volgnr_Col (keys @{$assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}[$volgnr_Group]->{Line}[$volgnr_Line]->{Col}}) {
                             ...
                             if (!$@) {
                               ...
                               if (!$@) {
                                    ...
                                    my $Description_warning= $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}[$volgnr_Group]->{Line}[$volgnr_Line]->{Warnings}->{Warning}->{Description};
                                    $Description_warning = substr ($Description_warning,0,235);
                                    $ExternalRef = "$code_warning : $Description_warning";
                                     $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                   }else {
                                    ...
                                    if (!$@) {
                                         ...
                                         my $Description_warning= $assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}[$volgnr_Group]->{Line}[$volgnr_Line]->{Warnings}[0]->{Warning}->{Description};
                                         $Description_warning = substr ($Description_warning,0,235);
                                         $ExternalRef = "$code_warning : $Description_warning";
                                          $ExternalRef = '' if ($ExternalRef eq ' : ') ;
                                        }else  {
...
sub maak_xml_invoice {
     my $xsd ='P:\OGV\ASSURCARD_PROG\asurcard_xsd\agresso\ABWTransaction-har.xsd';
     my $schema = XML::Compile::Schema->new($xsd);                      
     $schema->importDefinitions('P:\OGV\ASSURCARD_PROG\asurcard_xsd\agresso\ABWSchemaLib-har.xsd');#,
        ...
        #$schema->printIndex();
         ...
         #my $xml_file = "P:\\agressosoap\\test_invoice.xml";
...
sub send_via_webserv_client_invoice {
     ...
     my $soap = SOAP::Lite
     #-> proxy('http://10.198.205.8/AgressoWSHost/service.svc?ImportService/ImportV200606')10.198.216.91
     -> proxy('http://s200wr2xxl01.mutworld.be/BusinessWorld-webservices/service.svc?ImportService/ImportV200606') #productie
