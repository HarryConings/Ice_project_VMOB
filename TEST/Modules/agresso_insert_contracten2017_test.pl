...
#!versie met teller die enkel 's nacht draait
use strict;
use strict;
use Data::Dumper;
      ...
      foreach my $zkf (@ziekenfondsen) {
                if ($teller_in_welk_zkf_we_zitten >= $teller_in_welk_zkf_we_zaten) {
                         ...
                         my $sql =("SELECT a.AGRESONR,a.KNRN52,a.ZKF,c.EXIDL8,
                                        ...
                                        ORDER BY a.AGRESONR,b.ABTVKK ASC" );#fetch first 10 rows only AGRESONR != 0 
                         ...
                         while(@agresso_klant =$sth->fetchrow_array)  {
                             if ($agresso_klant[0] > $last_agr_nr) {                               
                                ...
                                if ($oud_agresso_nr != $agresso_klant[0] ) {
                                    ...
                                    &delete_contracten($dbh_agresso,$agresso_klant[0]);
                                    print '';
                                   }
...
sub delete_contracten {
      my ($dbh,$agresso_nr) =  @_;     
      my $client = 'VMOB';           
      $dbh->do("DELETE FROM afxvmobcontract WHERE client = '$client' and dim_value = $agresso_nr and CONVERT(date,last_update) <> CONVERT(date, getdate())");
      print "$dbh,$agresso_nr\n";
     }
sub backup_contract {
      my $dbh = shift @_;
      my $client = 'VMOB';
