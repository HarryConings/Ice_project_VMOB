...
def make_doc(place, location, customer):
    ...
    #de bestaande aandoeningen ophalen (als er zijn)
    counter = 0
    conditions_name = []
    conditions_text = []
    conditions_begin_date = []
    conditions_end_date = []
  
    try:
        conditions = agresso_client.conditions
        for i in conditions['condition']:
            if conditions['condition'][counter]['name_insurance'] != '':
                
                conditions_name.append(conditions['condition'][counter]['name_insurance'])
                conditions_text.append(conditions['condition'][counter]['text'])
                conditions_begin_date.append(conditions['condition'][counter]['begin_date'])
                conditions_end_date.append(conditions['condition'][counter]['end_date'])
    ...
    #zorgen dat de lengte minstens 5 is zodat alle bestaande aandoening velden in de brieven ingevuld kunnen worden
    if len(conditions_name) < 5:
        x = 5 - len(conditions_name)
        
        while x > 0:
            conditions_name.append('')
            conditions_text.append('')
            conditions_begin_date.append('')
            conditions_end_date.append('')
            
            x -= 1
            
    #ernstige ziekte ophalen (als er zijn)
    counter = 0
    diseases_name = []
    diseases_text = []
    ...
    #zorgen dat de lengte minstens 5 is zodat alle ernstige ziektes velden in de brieven ingevuld kunnen worden
    if len(diseases_name) < 5:
        x = 5 - len(diseases_name)
        
        while x > 0:
            diseases_name.append('')
            diseases_text.append('')
            
            x -= 1   
            
    #verzekeringen ophalen
    insurances = customer.data['verzekeringen']
    insurances_name = []
    
    for key in insurances:
        insurances_name.append(insurances[key]['naam'])
    
    #zorgen dat de lengte minstens 8 is zodat alle verzekering velden in de brieven ingevuld kunnen worden
    if len(insurances_name) < 8:
        x = 8 - len(insurances_name)
        
        while x > 0:
            insurances_name.append('')
    ...
    #agr_nr ophalen 
    agresso_id = agresso_client.agresso_id
    ...
    for tbl in doc.Tables:
        ...
        #weet nog niet goed wat in deze tabel moet komen
        ...
        #Alle verzekeringen van een persoon 
        ...
        #de bestaande aandoeningen van een persoon
    ...
    for shp in doc.Shapes:
        ...
        if shp.TextFrame.HasText:
            ...
            txtRng.Find.Execute('agr_nr', False, False, False, False, False, True, 1, False, agresso_id, 2)
