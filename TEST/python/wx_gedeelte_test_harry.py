...
@author: Tijs Conings
"""
from settings_xml_inlezen_test import automatic_letters_from_xml
...
class Frame(wx.Frame):
    ...
    def print_button(self, event):
        ...
        for key in Frame.text:
            ...
            #print ("text " + key + "  " + Frame.text[key])
            if r:
                naam_verzek = Frame.text[key]
                s = re.search ("\w+", naam_verzek)
                if s:
                    if teller_verz == 1:
                        customer.data['verzekeringen']={}
                    
                    customer.data['verzekeringen'][teller_verz]={}
                    customer.data['verzekeringen'][teller_verz]['naam']= Frame.text[key]
                    key_begin = re.sub("_naam","_begin", key, 1)
                    key_wacht = re.sub("_naam","_wacht", key, 1)    
                    key_eind = re.sub("_naam","_eind", key, 1)
                    key_zkf = re.sub("_naam","_zkf", key, 1)
                    #print ('key zkf ' + key_zkf)
                    key_dos_nr = re.sub("_naam","_dos_nr", key, 1)
                    customer.data['verzekeringen'][teller_verz]['begin']= Frame.text[key_begin]
                    customer.data['verzekeringen'][teller_verz]['wacht']= Frame.text[key_wacht]
                    customer.data['verzekeringen'][teller_verz]['eind']= Frame.text[key_eind]
                    customer.data['verzekeringen'][teller_verz]['zkf']= Frame.text[key_zkf]
                    customer.data['verzekeringen'][teller_verz]['dossier']= Frame.text[key_dos_nr]
                    teller_verz +=1
                    
        for key in Frame.checkbox_value:
            x = re.search("Verzekering\d_check", key)
            if x:
                '''print ('match verzekering' + key)'''
                if Frame.checkbox_value[key]:
                    print ('deze is true ' + key)
                    key_naam = re.sub("_check", "_naam", key, 1)
                    key_begin = re.sub("_check", "_begin", key, 1)
                    key_wacht = re.sub("_check", "_wacht", key,1)
                    key_eind = re.sub("_check", "_eind", key,1)
                    customer.data['gekozen_verzekering_naam'] = Frame.text[key_naam]
                    customer.data['gekozen_verzekering_begin']= Frame.text[key_begin]
                    customer.data['gekozen_verzekering_wacht']= Frame.text[key_wacht]
                    customer.data['gekozen_verzekering_eind'] = Frame.text[key_eind]
                    teller_verz_gekozen += 1
                #print (customer.data)
            y = re.search("aut_brieven\d\.\d_check", key)
            if y:
                '''print ('match welke automatische brief')'''
                if Frame.checkbox_value[key]:
                    key_brief = re.sub("_check", "", key, 1)
                    letter = Frame.text[key_brief]
                    if teller_brieven == 1:
                        customer.data['automatische_brieven'] ={}
                        customer.data['automatische_brieven'] ={teller_brieven:letter}
                    else:
                        customer.data['automatische_brieven'][teller_brieven]=letter
            ...
            if o:
                 '''print ('match welke mifid ontbrekende documenten')'''
                 if Frame.checkbox_value[key]:
                    key_mifid_odo =  re.sub("_check", "", key, 1)                    
                    mifid_odo = Frame.text[key_mifid_odo]
                    if teller_mifid_odo == 1:
                         customer.data['automatische_brieven_mifid_odo']={}
                         customer.data['automatische_brieven_mifid_odo']={teller_mifid_odo:mifid_odo}
                    else:
                         customer.data['automatische_brieven_mifid_odo'][teller_mifid_odo]=mifid_odo
                    teller_mifid_odo +=1 
            ...
            if p:
                if Frame.checkbox_value[key]:
                    ...
                    if teller_mifid_anm == 1:
                        customer.data['automatische_brieven_mifid_anm']= {}
                        customer.data['automatische_brieven_mifid_anm']={teller_mifid_anm:mifid_anm}
                    else:
                        customer.data['automatische_brieven_mifid_anm'][teller_mifid_anm]=mifid_anm
                    teller_mifid_anm +=1
            ...
            if q:
                if Frame.checkbox_value[key]:
                    ...
                    if teller_mifid_od == 1:
                         customer.data['automatische_brieven_mifid_od']={}
                         customer.data['automatische_brieven_mifid_od']={teller_mifid_od:mifid_od}
                    else:
                         customer.data['automatische_brieven_mifid_od'][teller_mifid_od]=mifid_od
                    teller_mifid_od += 1
        ...
        print (customer.data)  
        if teller_verz_gekozen > 0:
            '''ok er is een verzekering gekozen '''
            if teller_brieven >  1:
                '''ok er is een automatiche brief gekozen'''
            else:
...
def window2(event):
    ...
    notebook.Layout()
    #%%
    #alle knoppen en text voor het eerste tabblad -> Lid Opname Verzekering
    #%%
    ...
    #%%
    #Alle tekst en knoppen voor het tweede tabblat Bestaande aandoeningen
    #%%
    ...
    #%%
    #Alle tekst en knoppen voor het derde tabblat Ernstige ziekten
    #%%
    ...
    #%%
    #Alle tekst en knoppen voor het vierde tabblat GKD
    #%%
    ...
    #%%
    #Alle tekst en knoppen voor het vijfde tabblat Bieven Maken
    #%%
    ...
    #%%
    #Alle tekst en knoppen voor het zesde tabblat Automatische brieven
    #%%
    ...
    #Frame.checks['aut_brieven1.1_check'].SetValue(True)
    #x = Frame.checks
    #print ('in window')
    #print (Frame.checks)
    #%%
    ...
    window.check_box('mfid_od2.1_check', parent = window.tabs['MIFID ONTBREKENDE DOCUMENTEN'], pos = (720,35))
    ...
    #%%
    #Alle tekst en knoppen voor het achtste tabblat MIFID ONTBREKENDE DOCUMENTEN OMSCHAKELING
    #%%
    ...
    window.check_box('mfid_od_o2.1_check', parent = window.tabs['MIFID ONTBREKENDE DOCUMENTEN OMSCHAKELING'], pos = (720,35))
    ...
    #%%
    #Alle tekst en knoppen voor het negende tabblat MIFID AANSLUITING NOG NIET MOGELIJK
    #%%
    ...
    #%%
    window.buttons['Kies een verzekering en maak brief'].Bind(wx.EVT_BUTTON, open_file_dialog)
