...
def compare_insurances(customer):
    ...
    insurance_from_xml(customer, r'P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\Tijs.xml') #staat in het 'settings_xml_inlezen_test' bestand
    ins_for_agresso = []
    ...
    for insurance in customer.insurances:
        ...
        if p_type in insurance_from_xml.p_type_i:
            ...
            if p_code == '1':
                insurance['product'] = insurance_from_xml.insurances['T' + p_type] #p_type is de verzekering 
               
            else:
                insurance['product'] = insurance_from_xml.insurances['C' + p_code] #p_code is de subcode van de verzekering 
...
def date_formatter(date):
    '''datums in een dd/mm/yyyy formaat zetten'''
