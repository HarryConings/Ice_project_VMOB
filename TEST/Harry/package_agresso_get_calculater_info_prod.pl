...
sub variant_LG04 {
           ...
           #+trace => [ transport => sub { print $_[0]->as_string } ];
            ...
            my $template    = SOAP::Data->name('query:TemplateId' => "$TemplateId")->type(''); #prod
...
sub agresso_get_clients_with_assurcard_invoices {
      ...
      my $response ;
      my $faultcodecode ;
      eval {$response = $soap->mySOAPFunction($input,$credentials);}  or $faultcodecode = package_invoice_to_agresso->send_order_via_webserv_client('timeout');
      $main::progess_dialog->Update(5,'Agresso Response');
      #my @klanten;
      my @klantnaam;
      my $aantal_klanten = 0;
      eval {$faultcodecode =$response->{_content}[2][0][2][0][4]->{faultcode}};
      if (!$@) {
...
sub agresso_get_invoice_info {
      ...
      my $test =$main::invoice;
      #loop om hospiplan 882000 om te zetten naar 882001
      eval {foreach my $plan_inv (sort keys $main::invoice) {}};
      if (!$@) {
           foreach my $plan_inv (sort keys $main::invoice) {
                                 my $is_tweepersoonskamer =1;
                                 foreach my $plan_lijnnr (sort keys $main::invoice->{$plan_inv}) {
                                            $is_tweepersoonskamer =0 if ($main::invoice->{$plan_inv}->{$plan_lijnnr}->{interne_nomenclatuur} == 882206) ;   
                                 }
                                 print "\n\n_________________\n $is_tweepersoonskamer  is_tweepersoonskamer\n\n_________________\n";
                                 if ($is_tweepersoonskamer ==1) {
                                     foreach my $plan_lijnnr (sort keys $main::invoice->{$plan_inv}) {
                                                       if ($main::invoice->{$plan_inv}->{$plan_lijnnr}->{interne_nomenclatuur} == 882000) {
                                                            $main::invoice->{$plan_inv}->{$plan_lijnnr}->{interne_nomenclatuur} = 882001;       
                                                            #$main::invoice->{$plan_inv}->{$plan_lijnnr}->{hospiplan_tweepersoons} = 882001;    
                                                       }
                                            }      
                                 }
                      }
