...
package voor_en_nazorg_naar_agresso;
use Date::Manip::DM5 ;
use Date::Calc qw(:all);
use DateTime::Format::Strptime;
use DateTime;
sub welke_nomenclaturen_zijn_voor_en_nazorg {
     @main::Voor_en_NaZorg_nomenclaturen = ();
     #my %test = %main::nomenclatuurnummers_per_groep;
     foreach my $groep (keys %main::nomenclatuurnummers_per_groep) {
         if ($groep =~ m/VOOR\s+EN\s+NAZORG/) { #VOOR EN NAZORG - KINE en VOOR EN NAZORG
          #if ($groep eq 'VOOR EN NAZORG') {
             foreach my $nr (keys $main::nomenclatuurnummers_per_groep{$groep}) {
                 my $nomenclatuur = $main::nomenclatuurnummers_per_groep{$groep}->[$nr];
                 my $type_grid = $main::type_grid{$nomenclatuur};
                 if ($main::type_grid{$nomenclatuur} eq 'VnZ') {
                      push (@main::Voor_en_NaZorg_nomenclaturen,$nomenclatuur);
                    }
                }
            }
         
        }
     #my @test =@main::Voor_en_NaZorg_nomenclaturen;
     print "";
    }
sub save_Voor_en_Nazorg {
     my $agresso_nr = $main::klant->{Agresso_nummer};
     my $er_is_voor_en_nazorg =0;
     ...
     $xml=$xml."<cus:FlexiGroup>VMOBVZNZ</cus:FlexiGroup>";
     $xml=$xml."<cus:FlexiFieldRowList>";
     my $dbh = sql_toegang_agresso->setup_mssql_connectie;
     my $RowNo = sql_toegang_agresso->get_row_number_VMOBVZNZ($dbh,$agresso_nr);
     sql_toegang_agresso->disconnect_mssql($dbh);
     $RowNo =0 if (!defined $RowNo) ;
     #my @testnom =@main::Voor_en_NaZorg_nomenclaturen;
     #my $test =$main::overzicht_per_nomenclatuur;
     foreach my $nomenclatuur (@main::Voor_en_NaZorg_nomenclaturen) {
         foreach my $rij (keys $main::overzicht_per_nomenclatuur->{$nomenclatuur}) {
             ...
             $er_staat_iets_in_de_rij=1 if (defined $zkf_tussenkomst and $zkf_tussenkomst);
             ...
             my $beschrijving = '';
             #verandering ook rijen wanneer het maximum overschreden is !!!opgelet test in productie             
             if ($er_staat_iets_in_de_rij == 1 and ($nomenclatuur < 999991 or ($nomenclatuur > 999990 and ($hospi_tussenkomst != 0 or $verschil !=0)) )) {
                 # if   ($rij != $laaste_rij) {
                 if (($pers_tussenkomst > 0 or $zkf_tussenkomst >0 or $hospi_tussenkomst > 0) and $rij != $laaste_rij) {
                     $er_is_voor_en_nazorg =1;
                ...
                #}elsif ($er_staat_iets_in_de_rij == 1 and $nomenclatuur < 999991) {   #verandering ook rijen wanneer het maximum overschreden is !!!opgelet test in productie
                #         if   ($rij != $laaste_rij) {
                #              $er_is_voor_en_nazorg =1;
                #              $begindatum = $begindatum_alg if (!defined $begindatum or $begindatum eq '') ;
                #              $einddatum = $einddatum_alg if (!defined $einddatum or $einddatum eq '') ;
     ...
     $xml=$xml."</cus:credentials>";
     my ($returncode,$ReturnText) = ("OK","geen voor en nazorg");
     ($returncode,$ReturnText) = &insert_contracten($xml) if ($er_is_voor_en_nazorg ==1) ;
     my $historiek_gkd = $main::teksten_GKD->{GKD_teksten}->{AUTOMATISCHE_TEKSTEN}->{VOOR_EN_NAZORG}->{tekst};
     my $staat_er_al_in = 'nee';
     $staat_er_al_in = as400_gegevens->lees_history_gkd_agresso_order($historiek_gkd) if ($er_is_voor_en_nazorg ==1) ;
     as400_gegevens->zet_history_gkd_in ($historiek_gkd) if ($er_is_voor_en_nazorg ==1 and $staat_er_al_in eq 'nee') ;
