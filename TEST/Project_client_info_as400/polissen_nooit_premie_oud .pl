...
package AS400;
    ...
    #A.W010L8              NOM/PRENOM ALPHABETIQUE /NAAM/      
      ...
      foreach my $zkf (keys $instellingen->{ziekenfondsen}) {
                         ...
                         my $sql =("SELECT c.KNRNL8,c.EXIDL8,
                                        ...
                                        ORDER BY  b.ABNOKK DESC fetch first 30 rows only " );#fetch first 10 rows only
    ...
    sub check_betaling_dossier {
        ...
        foreach my $zkf_nr (sort keys $klanten) {
                  ...
                  foreach my $dossier (sort keys $klanten->{$zkf_nr}) {
                      my $dossier_niet_betaald = 1;
                      foreach my $ext_nr (keys $klanten->{$zkf_nr}->{$dossier} ) {
                          if ($dossier == 203229658) {
                            print '';
                          }
                          my ($jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles)= AS400->checkbetaling($dbh,$PTAXKQ,$zkf_nr,$dossier, $ext_nr);
                          print "$jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles\n";
                          if ($hebben_nooit_betaald == 0) {
                             $dossier_niet_betaald =0 ;
                             print "$dossier betaald\n";
                             last;
                          }
                          
                        }
                      
                      if ($dossier_niet_betaald == 1) {
                         foreach my $ext_nr (keys $klanten->{$zkf_nr}->{$dossier} ) {  
    ...
    sub checkbetaling {
     my ($self,$dbh,$betaling_fil,$nr_zkf,$dossier,$externnummer) = @_ ;   
     ...
     #ABNOKQ            osrriernr
     my $sqlbetaling =("SELECT IDFDKQ,EXIDKQ,ABTVKQ,ABVYKQ,ABVMKQ,ABCNKQ,ABCOKQ,AT79KQ FROM $betaling_fil WHERE IDFDKQ = $nr_zkf and EXIDKQ  = $externnummer and ABNOKQ  = $dossier ");
...
package Frame;
     ...
     sub new {
               ...
               #RIJ 2
               #$main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               #$main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_statictxt_Extern}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               #$main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_statictxt_INZ}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               #$main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_statictxt_Zoek}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
