...
our $peilings_datum_ok;
our $start_datum_ok;
our $klanten;
our $mail_as400 ='';
our $eind_datum;
our $zes_maanden_geleden;
our $instellingen;
package main;
 $instellingen = XMLin('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\Poject_AS400_settings.xml');
 my $app = App_standaard->new;          
 $app->MainLoop;
 
 if ($peilings_datum_ok > 20190000) {
    $eind_datum =$peilings_datum_ok;
    if  ($start_datum_ok > 19010000) {
        $zes_maanden_geleden = $start_datum_ok;
    }else {
  ...
  print "";
    ...
    my $volledig_niet_betaalde_dossiers = AS400->check_betaling_dossier($instellingen,$klanten);
   
    my $gedaan = maakExcel->rapport2($klanten,$instellingen,$eind_datum,$volledig_niet_betaalde_dossiers);
    print '';
1;
...
package App_standaard;
     ...
     sub OnInit {
			...
			$main::dialog->SetSize(1, 1, 600, 170);
			$main::dialog->Centre();
			$main::dialog->Show(1);
#         #$main::dialog =Frame->voorstelafdrukken_uitkeringen();
...
package Frame;
     ...
     sub new {
               ...
               my $peilings_datum;
               my $start_datum;
               $main::frame = $self->SUPER::new(undef, -1,_T("Polissen nooit premie betaald"),
                                        [-1,-1],[600,170], wxDEFAULT_FRAME_STYLE | wxTAB_TRAVERSAL  );
                
               $main::frame->{Frame_Sizer_1} = Wx::FlexGridSizer->new(3,4, 10, 10);
               $main::frame->{Frame_statictxt_waar}= Wx::StaticText->new($main::frame, 105,_T("$instellingen->{excel}->{plaats_verslag}"),wxDefaultPosition,wxSIZE(250,20));
               $main::frame->{Frame_statictxt_peilings_datum}= Wx::StaticText->new($main::frame, 101,_T("Peilings Datum"),wxDefaultPosition,wxSIZE(170,20));
               $main::frame->{Frame_txt_peilings_datum} = Wx::TextCtrl->new($main::frame, 102, $peilings_datum,wxDefaultPosition,wxSIZE(170,20));
               $main::frame->{Frame_statictxt_Datum_start_contract}= Wx::StaticText->new($main::frame, 107,_T("Datum start Contract"),wxDefaultPosition,wxSIZE(170,20));
               $main::frame->{Frame_txt_Datum_start_contract} = Wx::TextCtrl->new($main::frame, 108, $start_datum,wxDefaultPosition,wxSIZE(170,20));
               $main::frame->{Frame_Cancel}  = Wx::Button->new($main::frame, 103, _T("Cancel"),wxDefaultPosition,wxSIZE(170,20));
               $main::frame->{Frame_Ok}  = Wx::Button->new($main::frame, 104, _T("OK"),wxDefaultPosition,wxSIZE(170,20));              
               $main::frame->{Frame_panel_1} = Wx::Panel->new($main::frame,-1,wxDefaultPosition,wxSIZE(25,5));
             
               
               #rij0
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_statictxt_waar}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               ...
               #rij1
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_statictxt_peilings_datum}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_statictxt_Datum_start_contract}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
              
               #RIJ2
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_txt_peilings_datum}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_txt_Datum_start_contract}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);              
               ...
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_Ok}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);	   
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_Cancel}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
               $main::frame->{Frame_Sizer_1}->Add($main::frame->{Frame_panel_1}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT); 
	  
               Wx::Event::EVT_BUTTON($main::frame,$main::frame->{Frame_Ok},\&Ok); 
               Wx::Event::EVT_BUTTON($main::frame,$main::frame->{Frame_Cancel},\&Cancel); 
               $main::frame->SetSizer($main::frame->{Frame_Sizer_1});
               $main::frame->SetBackgroundColour(Wx::Colour->new(239, 243, 255));
               #$main::frame->Close();
               return ($main::frame);
          }
     sub Ok {
         $peilings_datum_ok =$main::frame->{Frame_txt_peilings_datum}->GetValue;
         $start_datum_ok =$main::frame->{Frame_txt_Datum_start_contract}->GetValue;
         $app->ExitMainLoop;
     }
     sub Cancel {
        die;
...
package maakExcel;
     ...
     use Unicode::String qw(utf8 55nd6r6n
     utf16le);
     ...
     sub rapport1 {
          ...
          my $naam_verslag ="Polisten_nooit_premie_$eind_datum\.xlsx";
...
#                                            nieuw stuk bijgevoegd
 ...
 sub rapport2 {
          my ($self,$klant,$instellingen,$eind_datum,$volledig_niet_betaalde_dossiers) = @_;
          my @slechte_betalers;
          eval {            foreach my $inz_bet (sort keys $volledig_niet_betaalde_dossiers) {}};        
           
          my $plaats_sjabloon = $instellingen->{excel}->{plaats_sjabloon};
          my $sjabloon2 =  $instellingen->{excel}->{sjabloon2};
          my $sjabloon = "$plaats_sjabloon\\$sjabloon2";
          my $plaats_verslag = $instellingen->{excel}->{plaats_verslag};
          my $naam_verslag ="Polissen_nooit_premie_$eind_datum\.xlsx";
          unlink "$plaats_verslag\\$naam_verslag";
          my $Excel = Win32::OLE->GetActiveObject('Excel.Application')
     	|| Win32::OLE->new('Excel.Application', 'Quit');     
          $Excel->{'Visible'} = 0;  # toon wat je doet is 1
          ...
          my $kolom_offset = 1;
          my $inhoudcel= $Sheet->Cells($rij_offset,$kolom_offset+2);
          $inhoudcel->{Value} = "Polissen nooit premie op $eind_datum";
          $inhoudcel= $Sheet->Cells($rij_offset+2,$kolom_offset+0);
          $rij_teller +=1;
          #titel
          ...
          my $kolom_adres=16;
          my $er_is_een_verzekeringslijn=0;          
          foreach my $zkfnr (sort keys $klanten) {
              $rij_teller +=1 if ($er_is_een_verzekeringslijn==0);
              $aantal_verzekerden +=1; 
              eval {foreach my  $doss_nr (keys $klanten->{$zkfnr}) {}};
                if (!$@){
                  foreach my $doss_nr (keys $klanten->{$zkfnr}) { 
                    
                         $er_is_een_verzekeringslijn=0;
                         eval  {foreach my $extern_nummr (sort keys $klanten->{$zkfnr}->{$doss_nr}) {}};
                         if (!$@){
                                foreach my $extern_nummr (sort keys $klanten->{$zkfnr}->{$doss_nr}) {  
                                    eval  {foreach my $verzekering (sort keys $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}) {}};
                                    foreach my $verzekering (sort keys $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}) {
                                       my $verzekerings_nummer=0;
                                       if ($verzekering ne 'klantinfo') {
                                            my $inz_test =$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{INZ};
                                            $verzekerings_nummer=$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{verzekeringsnr};
                                            my @verzekeringen_nietbetaald;
                                            eval {foreach my $verz_nooit_betaald (keys $volledig_niet_betaalde_dossiers->{$inz_test}->{$zkfnr}->{klantinfo}->{nooit_betaald}) {} };
                                            if (!$@) {
                                             foreach my $verz_nooit_betaald (keys $volledig_niet_betaalde_dossiers->{$inz_test}->{$zkfnr}->{klantinfo}->{nooit_betaald}) {
                                                push (@verzekeringen_nietbetaald,$verz_nooit_betaald);
                                                my $ucverz_nooit_betaald = uc $verz_nooit_betaald;
                                                push (@verzekeringen_nietbetaald,$ucverz_nooit_betaald);   
                                             }
                                             
                                            }
                                            if ($verzekerings_nummer ~~   @verzekeringen_nietbetaald) {
                                                  #nieuw stuk bijgevoegd van bovenstaande om alle cellen in te vullen
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+1);
                                                   $inhoudcel->{Value} =  sprintf ('%011s',$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{INZ});
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset +2);
                                                   $inhoudcel->{Value} =$zkfnr;
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset +3);
                                                   $inhoudcel->{Value} =$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{voor_naam};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset +4);                                           
                                                   $inhoudcel->{Value} =$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{naam};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset +5);
                                                   $inhoudcel->{Value} =$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{geslacht};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset +6);
                                                   $inhoudcel->{Value} =$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{ouderdom};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset +7);
                                                   $inhoudcel->{Value} =$klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{geboortedatum};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres);   
                                                   $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{D_Address};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+1);   
                                                   $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{D_ZipCode};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+2);   
                                                   $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{klantinfo}->{D_Place};
                                                   $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+3);   
                                                   $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{klantinfo}->{D_CountryCode};
                                                   #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+4);
                                                   #$inhoudcel->{Value} =  $klanten->{$zkfnr}->{$doss_nr}->{$verzekering}->{openstaand_saldo};
                                                   #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+5);
                                                   #$inhoudcel->{Value} =  $klanten->{$zkfnr}->{$doss_nr}->{$verzekering}->{gefactureerd};
                                                   #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+6);
                                                   #$inhoudcel->{Value} =  $klanten->{$zkfnr}->{$doss_nr}->{$verzekering}->{saldo};
                                                   #$inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{klantinfo}->{P_Address};
                                                   #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+5);   
                                                   #$inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{klantinfo}->{P_ZipCode};
                                                   #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+6);   
                                                   #$inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{klantinfo}->{P_Place};
                                                   #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+7);   
                                                   #$inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{klantinfo}->{P_CountryCode};
       ...
       #                                            nieuw stuk bijgevoegd
                                                     ...
                                                     $inhoudcel->{Value} = $verzekering;
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+9);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{dossier_nr};
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+10);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{aansluitingscode};
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+11);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{startdatum};
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+12);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{wachtdatum};
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+13);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{ontslagcode};
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+14);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{einddatum};
                                                     $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+15);
                                                     $inhoudcel->{Value} = $klanten->{$zkfnr}->{$doss_nr}->{$extern_nummr}->{$verzekering}->{Verz_nr};
                                                     $rij_teller +=1;
                                                     $er_is_een_verzekeringslijn=1; 
                                            }
#                                          
...
package AS400;
    ...
    #A.W010L8              NOM/PRENOM ALPHABETIQUE /NAAM/      
      ...
      foreach my $zkf (keys $instellingen->{ziekenfondsen}) {
                         ...
                         my $sql =("SELECT c.KNRNL8,c.EXIDL8,
                                        ...
                                        ORDER BY  b.ABNOKK DESC fetch first 10 rows only" );#fetch first 10 rows only c.KNRNL8 != 0
                         ...
                         while(@agresso_klant =$sth->fetchrow_array)  {
                                ...
                                #my ($jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles)= AS400->checkbetaling($dbh,$PTAXKQ,$zkf_nr,$agresso_klant[2],$agresso_klant[1]);
                                #print "saldo $saldob  bedrag betaald $bedragb $totb\n" if ($saldob > 10);
                                #if ($hebben_nooit_betaald == 1) {
                                        ...
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{INZ} = $agresso_klant[0];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{naam} = $agresso_klant[12];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{voor_naam} = $agresso_klant[13];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{geboortedatum} ="$agresso_klant[16]/$agresso_klant[15]/$agresso_klant[14]";
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{ouderdom} =main->ouderdom("$agresso_klant[14]-$agresso_klant[15]-$agresso_klant[16]");
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{geslacht} = '';
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{geslacht} = 'man' if $agresso_klant[17] == 01;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{geslacht} = 'vrouw' if $agresso_klant[17] == 02;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{D_Address}= $D_Address;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{D_ZipCode}= $D_ZipCode;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{D_Place}= $D_Place;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{D_CountryCode}= $D_CountryCode;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{P_Address}= $P_Address;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{P_ZipCode}= $P_ZipCode;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{P_Place}= $P_Place;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{klantinfo}->{P_CountryCode}= $P_CountryCode;
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{startdatum} =$agresso_klant[4];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{verzekeringsnr} =$agresso_klant[2];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{dossier_nr}  =$agresso_klant[7];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{Verz_nr} = "$agresso_klant[2] P $agresso_klant[3]";
                                       #  $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{Verz_nr} = "$agresso_klant[2] P $agresso_klant[3]"; 
                                        #$klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{openstaand_saldo} = $totaal_over_alles;
                                        ...
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{wachtdatum} = $agresso_klant[5];
                                        ...
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{aansluitingscode}= "$agresso_klant[8]$agresso_klant[9]";
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{ontslagcode}= "$agresso_klant[10]$agresso_klant[11]";
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{einddatum}=$agresso_klant[6];
                                        $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$verz_naam}->{datestring} = gmtime();
                                        #print "time $klant->{$zkf_nr}->{$agresso_klant[7]}->{$agresso_klant[1]}->{$naam}->{datestring}\n";
                                        $line_no +=1;
                                        $totaal_aantal_lijnen +=1;
                                        my $zkf_line_no=$zkf_nr*100+ $line_no;
                                #}
                               
                               #$line_no = $dbh_agresso->selectrow_array("SELECT COUNT(*) from afxvmobcontract where dim_value= '$agresso_klant[1]'");
                            }
    ...
    sub check_betaling_dossier {
        my ($self,$instellingen,$klanten) = @_;
        my $alle_dossiers_niet_betaald;
                            ...
                            foreach my $verz_naam (keys $klanten->{$zkf_nr}->{$dossier}->{$ext_nr} ) {
                                  if ($verz_naam !~ m/klantinfo/i) {
                                    my $vz_nr = $klanten->{$zkf_nr}->{$dossier}->{$ext_nr}->{$verz_naam}->{verzekeringsnr};
                                     push (@verzekeringen_genomen,$vz_nr);
                                  }
                              }
                           foreach my $verz_nr (@verzekeringen_genomen) {                              
                                    my ($jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles)= AS400->checkbetaling($dbh,$PTAXKQ,$zkf_nr,$dossier, $ext_nr,$verz_nr);
                                    print "$dossier verz_nr -> $verz_nr $jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles\n";
                                    if ($hebben_nooit_betaald == 1) {
                                       $dossier_niet_betaald->{$verz_nr}->{$ext_nr} =1 ;
                                       print "$dossier -> $verz_nr $ext_nr NIET  betaald\n";
                                       #last;
                                    }else {
                                       $dossier_niet_betaald->{$verz_nr}->{$ext_nr} =0 ;
                                       print "$dossier -> $verz_nr $ext_nr WEL betaald\n";
                                    }
                              }
                           foreach  my $verz_nr (sort keys $dossier_niet_betaald) {
                                 $dossier_niet_betaald->{$verz_nr}->{helemaal_niet_betaald}=1;
                                 foreach my $exnr (sort keys $dossier_niet_betaald->{$verz_nr}) {
                                      $dossier_niet_betaald->{$verz_nr}->{helemaal_niet_betaald}=0 if ($dossier_niet_betaald->{$verz_nr}->{$exnr} ==0 );
                                 }
                               
                              }      
                           
                        }
                      eval {foreach my $verz_naam_niet_betaald (keys $dossier_niet_betaald) {} };
                      if (!$@ ) {
                         foreach my $verz_naam_niet_betaald (keys $dossier_niet_betaald) {
                           if ($dossier_niet_betaald->{$verz_naam_niet_betaald}->{helemaal_niet_betaald} == 1) {
                              foreach my $ext_nr (keys $klanten->{$zkf_nr}->{$dossier} ) {  
                                  my $inz =$klanten->{$zkf_nr}->{$dossier}->{$ext_nr}->{klantinfo}->{INZ} ;
                                  $alle_dossiers_niet_betaald->{$inz}->{$zkf_nr}->{klantinfo}->{naam} =  $klanten->{$zkf_nr}->{$dossier}->{$ext_nr}->{klantinfo}->{naam} ;
                                  $alle_dossiers_niet_betaald->{$inz}->{$zkf_nr}->{klantinfo}->{voornaam} =$klanten->{$zkf_nr}->{$dossier}->{$ext_nr}->{klantinfo}->{voor_naam} ;
                                  $alle_dossiers_niet_betaald->{$inz}->{$zkf_nr}->{klantinfo}->{nooit_betaald}->{$verz_naam_niet_betaald} =$dossier;
                                  print '';
                                 }
                           }
     ...
     return ( $alle_dossiers_niet_betaald);  
    }
    sub checkbetaling {
     my ($self,$dbh,$betaling_fil,$nr_zkf,$dossier,$externnummer,$verz_nr) = @_ ;   
     ...
     #ABNOKQ            osrriernr
     my $sqlbetaling =("SELECT IDFDKQ,EXIDKQ,ABTVKQ,ABVYKQ,ABVMKQ,ABCNKQ,ABCOKQ,AT79KQ FROM $betaling_fil WHERE IDFDKQ = $nr_zkf and EXIDKQ  = $externnummer and ABNOKQ  = $dossier and ABTVKQ =$verz_nr ");
     ...
     if  ($rijenteller > 0 ) {
         foreach $k (sort keys $betalingenh) {
            ...
            if (($k > $kold) and  ($betalingenh->{$k}->{'saldo'} < $betalingenh->{$k}->{'bedrag'} ) ){
                $kold = $k;
                #print "->tweede -kold $kold \n";
                $jaarb = $betalingenh->{$k}->{'jaar'} ;
                $maandb = $betalingenh->{$k}->{'maand'};
     ...
     return ($jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles);
    }
