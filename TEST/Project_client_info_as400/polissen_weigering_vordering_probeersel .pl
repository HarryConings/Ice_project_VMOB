...
#my $klanten = AS400->zoek_verzekerden($instellingen,$verzekeringen,$eind_datum,$begin_datum);
my $klanten = AS400->checkbetaling();
...
1;
    ...
    sub ouderdom {
     ...
     sub begin_jaar {
        my ($self,$datum) = @_;
        my $jaar= substr($datum,0,4);
        my $begindatum = $jaar*10000+100+1;
        return ($begindatum);
     }
package AS400;
    use Unicode::String qw(utf8 latin1 utf16le);
    sub zoek_verzekerden {
     my ($self,$instellingen,$verzekeringen,$eind_datum,$begin_jaar) = @_;
     my $klant;
    ...
    #A.W010L8              NOM/PRENOM ALPHABETIQUE /NAAM/      
      ...
      foreach my $zkf (keys $instellingen->{ziekenfondsen}) {
                         ...
                         my $sql =("SELECT c.KNRNL8,c.EXIDL8,
                                        ...
                                        and  AB2OKK='06' 
                                        ORDER BY  c.KNRNL8 ASC fetch first 100 rows only " );#fetch first 10 rows only and b.ABADKK = $begin_jaar b.ABOCKK = 'G' and
                         ...
                         while(@agresso_klant =$sth->fetchrow_array)  {
                                ...
                                my ($jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles,$totaal_saldo)= AS400->checkbetaling($dbh,$PTAXKQ,$zkf_nr,$agresso_klant[2],$agresso_klant[1]);
                                ...
                                if ($totaal_saldo >=0) {
                                        ...
                                        $klant->{$agresso_klant[0]}->{$zkf_nr}->{klantinfo}->{geslacht} = 'man' if $agresso_klant[17] == 01;
                                        $klant->{$agresso_klant[0]}->{$zkf_nr}->{klantinfo}->{geslacht} = 'vrouw' if $agresso_klant[17] == 02;
                                        ...
                                        $klant->{$agresso_klant[0]}->{$zkf_nr}->{$verz_naam}->{dossier_nr}  =$agresso_klant[7];
                                        ...
                                        $klant->{$agresso_klant[0]}->{$zkf_nr}->{$verz_naam}->{totaal_saldo} = $totaal_saldo;
    ...
    sub checkbetaling {
     my ($self,$dbh,$betaling_fil,$nr_zkf,$type_verz,$externnummer,$verzekeringen) = @_ ;
     my $placeholders = join ",", (@{$verzekeringen->{"ZKF$nr_zkf"}});
    ...
    #     SELECT IDFDKQ,EXIDKQ,ABTVKQ,SUM(ABCOKQ) FROM libcxfil03/PTAXKQ WHERE    
    # IDFDKQ = 203 and ABTVKQ  IN (52, 51, 61, 62) and ABPYKQ= '06'          
    #GROUP BY IDFDKQ,EXIDKQ,ABTVKQ order by EXIDKQ                           
    #SELECT statement run complete.                                        
                                                                         
     my $sqlbetaling =("SELECT IDFDKQ,EXIDKQ,ABTVKQ,SUM(ABCOKQ) FROM $betaling_fil WHERE IDFDKQ = $nr_zkf and ABTVKQ  IN $placeholders and ABPYKQ= '06'  GROUP BY IDFDKQ,EXIDKQ,ABTVKQ order by EXIDKQ ");
     my $sthbetaling = $dbh->prepare( $sqlbetaling );
     $sthbetaling ->execute();
     my $klant;
     while(my @betalingen =$sthbetaling ->fetchrow_array)  {
         $klant->{$betalingen[1]}->{$nr_zkf}->{$betalingen[2]}->{saldo} = $betalingen[3];
     }
     return ();
    }
package maakExcel;
...
#                                            nieuw stuk bijgevoegd van bovenstaande om alle cellen in te vullen
                                            ...
                                            $inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+4);
                                            $inhoudcel->{Value} =  $klanten->{$rrn}->{$zkfnr}->{$verzekering}->{totaal_saldo};
                                            #$inhoudcel= $Sheet->Cells($rij_offset+$rij_teller,$kolom_offset+$kolom_adres+5);
