...
my $klanten = AS400->zoek_verzekerden($instellingen,$verzekeringen,$eind_datum);
my $gedaan = maakExcel->rapport1($klanten,$instellingen,$eind_datum);
print '';
1;
...
package AS400;
    ...
    sub zoek_verzekerden {
     my ($self,$instellingen,$verzekeringen,$eind_datum) = @_;
     my $klant;
    ...
    #A.W010L8              NOM/PRENOM ALPHABETIQUE /NAAM/      
      ...
      foreach my $zkf (keys $instellingen->{ziekenfondsen}) {
                         ...
                         my $sql =("SELECT c.KNRNL8,c.EXIDL8,
                                        ...
                                        and b.ABEDKK >= $eind_datum and  b.ABADKK <= $eind_datum
                                        ORDER BY  c.KNRNL8 ASC " );#fetch first 10 rows only
                         ...
                         while(@agresso_klant =$sth->fetchrow_array)  {
                                ...
                                my $naam_spaties = $agresso_klant[12];
                                $naam_spaties=~ s/^\s+|\s+$//g;
                                my $voornaam_spaties = $agresso_klant[13];
                                $voornaam_spaties=~ s/^\s+|\s+$//g;
                                $klant->{$agresso_klant[0]}->{$zkf_nr}->{klantinfo}->{naam} = "$voornaam_spaties $naam_spaties";
                                ...
                                my $startdatum_jaar = substr($agresso_klant[4],0,4);
                                my $startdatum_maand = substr($agresso_klant[4],4,2);
                                my $startdatum_dag = substr($agresso_klant[4],6,2);
                                $klant->{$agresso_klant[0]}->{$zkf_nr}->{$verz_naam}->{startdatum} ="$startdatum_dag-$startdatum_maand-$startdatum_jaar";
                                ...
                                my $wachtdatum_jaar = substr($agresso_klant[5],0,4);
                                my $wachtdatum_maand = substr($agresso_klant[5],4,2);
                                my $wachtdatum_dag = substr($agresso_klant[5],6,2);
                                $klant->{$agresso_klant[0]}->{$zkf_nr}->{$verz_naam}->{wachtdatum} = "$wachtdatum_dag-$wachtdatum_maand-$wachtdatum_jaar";
                                ...
                                my $einddatum_jaar = substr($agresso_klant[6],0,4);
                                my $einddatum_maand = substr($agresso_klant[6],4,2);
                                my $einddatum_dag = substr($agresso_klant[6],6,2);
                                $klant->{$agresso_klant[0]}->{$zkf_nr}->{$verz_naam}->{einddatum}="$einddatum_dag-$einddatum_maand-$einddatum_jaar";
