...
package AS400;
    ...
    #A.W010L8              NOM/PRENOM ALPHABETIQUE /NAAM/      
      ...
      foreach my $zkf (keys $instellingen->{ziekenfondsen}) {
                         ...
                         my $sql =("SELECT c.KNRNL8,c.EXIDL8,
                                        ...
                                        and b.ABEDKK < $eind_datum 
                                        ORDER BY  c.KNRNL8 ASC " );#fetch first 10 rows only enddatum ? and b.ABEDKK > $eind_datum and  b.ABADKK <= $zes_maanden_geleden
                         ...
                         while(@agresso_klant =$sth->fetchrow_array)  {
                                ...
                                my ($jaarb,$maandb,$bedragb,$saldob,$totb,$hebben_nooit_betaald,$totaal_over_alles,$weigering,$dossier)= AS400->checkbetaling($dbh,$PTAXKQ,$zkf_nr,$agresso_klant[2],$agresso_klant[1],$agresso_klant[7],$eind_datum);
                                #print "$weigering saldo $saldob  bedrag betaald $bedragb $totb\n" ;#if ($weigering eq 'Weigering');
                                if ($weigering eq 'Weigering' and $saldob < 2 and $totb >= 0) {
    ...
    sub checkbetaling {
     my ($self,$dbh,$betaling_fil,$nr_zkf,$type_verz,$externnummer,$doss_ier,$eind_datum) = @_ ;
     my $nu_jaar = substr ($eind_datum,0,4);
     ...
     my $sqlbetaling =("SELECT IDFDKQ,EXIDKQ,ABTVKQ,ABVYKQ,ABVMKQ,ABCNKQ,ABCOKQ,AT79KQ,ABPYKQ,ABNOKQ  FROM $betaling_fil WHERE IDFDKQ = $nr_zkf and EXIDKQ  = $externnummer and ABTVKQ  = $type_verz
                       and ABPYKQ = 06 and ABVYKQ BETWEEN 2018 and $nu_jaar and ABNOKQ = $doss_ier");
     ...
     if (!$@) {
       ...
       if ($ABPYKQ eq 'Weigering' and $saldob < 2 and $totb >= 0) {
           print '';
       }
