...
package main;
     ...
     main->load_agresso_setting("D:\\OGV\\ASSURCARD_2023\\assurcard_settings_xml\\maf_agresso_settings.xml"); #nagekeken
     ...
     sub maak_betalingen_aan  {
         ...
         if ($nomenclatuur ne '') {
              ...
              print "\nBerekeningsjaren : $eerstejaar,$tweedejaar,$derdejaar\n";
              my $dbh_agresso = sql_toegang_agresso->setup_mssql_connectie ($main::mode);
...
package Frame;
     ...
     #odfLocalEncoding('iso-8859-15');  #versie 5.2
      sub new {
               ...
               $frame = $frame->SUPER::new(undef, -1,_T("Berekeningsjaren Agresso $main::mode -> ip $ip ->  variant = $variant_LG04 V20240112"),
     ...
     sub mail_bericht {
         ...
         $vandaag = sprintf "%04d-%02d-%02d",substr ($vandaag,0,4),substr ($vandaag,4,2),substr ($vandaag,6,2);
          foreach my $geadresseerde (@aan_lijst) {
             #my $smtp = Net::SMTP->new('10.63.120.3',Hello => 'mail.vnz.be',Timeout => 60);
             my $smtp = Net::SMTP->new('mailservices.m_team.be',Hello => 'mail.vnz.be',Timeout => 60);
...
package excel;
     ...
     sub new {     
         ...
         my($nepexcel,$file_xlsx) = excel->save_excel($Excel1);
         my $Excel = Win32::OLE->GetActiveObject('Excel.Application')
        	|| Win32::OLE->new('Excel.Application', 'Quit');
         $Excel->{'Visible'} = 1;
         my $Book = $Excel->Workbooks->Open($file_xlsx);
     ...
     sub save_excel {
         ...
         unlink "$path_to_files\\$vandaag-MAF.xlsx";
         $Excel->ActiveWorkbook->SaveAs("$path_to_files\\$vandaag-MAF.xlsx");
         $Excel->ActiveWorkbook->Close(1);
         $Excel->Quit();
         return ($Excel,"$path_to_files\\$vandaag-MAF.xlsx");
        }
     sub save_excel_file {
          my ($self,$Excel) = @_;
