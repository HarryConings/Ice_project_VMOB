...
package main;
     our @Doctype_Agresso = ();
     our $version = 'v20231017/v7';
     our $mode = 'TEST';
     $mode = $ARGV[0] if (defined $ARGV[0]);
     if ( $mode eq 'TEST' or $mode eq 'PROD'){}else{die}
     our $instellingen = main->load_settings('D:\OGV\ASSURCARD_2023\assurcard_settings_xml\doccenter_to_agresso.xml'); #C:\macros\doccentermail
...
package Frame;
     ...
     sub new {
               ...
               $frame = $frame->SUPER::new(undef, -1,_T("Mode: $main::mode - Version: $main::version - Welke Doccenter brief wil je in agresso $mode zetten"),
...
package webservice;
     ...
     sub PDF_naar_Agresso {
         ...
         my $prox = $instellingen->{"Agresso_IP_$main::mode"};
         my $proxy = "http://$prox/BusinessWorld-webservices/service.svc?CustomerService/Customer";# test
     ...
     sub agresso_get_customer_info_rr_nr {
      ...
      my $prox = $instellingen->{"Agresso_IP_$main::mode"};
      my $proxy = "http://$prox/BusinessWorld-webservices/service.svc?CustomerService/Customer";     
