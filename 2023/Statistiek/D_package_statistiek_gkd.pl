...
package main;
    ...
    sub load_statistiek_gkd_instellingen  {
         ...
         print "$file_name ->";
         $statistiek_gkd_instellingen = XMLin('D:\OGV\ASSURCARD_2023\assurcard_settings_xml\statistiek_gkd_settings.xml');
         foreach my $nr (keys  $statistiek_gkd_instellingen->{GKD_teksten}->{TABBLAD_GKD}->{tekst}) {
             push (@tekst_match, $statistiek_gkd_instellingen->{GKD_teksten}->{TABBLAD_GKD}->{tekst}->[$nr]);
         }
          print "ingelezen\n";
