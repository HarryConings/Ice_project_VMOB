...
sub get_contracts_by_type {
       ...
       #17 zkf_nr_datum_van
        ...
        my $sql =("SELECT dim_value,product,contract_nr,startdatum,wachtdatum,einddatum,zkf_nr FROM afxvmobcontract WHERE product ='$type'
                  and client = 'VMOB' and startdatum < '$startdatum' order by dim_value,startdatum ");  #test overgang dim_value = '210411'
        my $sth = $dbh->prepare( $sql );
        $sth->execute();
         while(my @rij =$sth->fetchrow_array)  {
           #print"@rij\n";
           #PTL Wordt TIT
           my $neem_de_laatse = 1;
           if ($contracten->{$rij[0]}->{contract_nr}) {
               my $begin_datum2=substr($rij[3],0,10); 
               if ($begin_datum2 > $contracten->{$rij[0]}->{begin_datum}) {                 
                   my ($year,$month,$day) =  Date::Calc::XS::Add_Delta_Days(split(/-/,$contracten->{$rij[0]}->{eind_datum}), 1); #de volgende dag
                   $year =  sprintf ('%04s',$year);
                   $month = sprintf ('%02s',$month);
                   $day = sprintf ('%02s',$day);
                   my $berekende_startdatum_nieuw_contract = "$year-$month-$day";
                   if ($berekende_startdatum_nieuw_contract eq $begin_datum2) {
                    ...
                    my ($year,$month,$day) =   Date::Calc::XS::Add_Delta_Days(split(/-/,substr($rij[5],0,10)), 1); #de volgende dag
                    $year =  sprintf ('%04s',$year);
                    $month = sprintf ('%02s',$month);
                    $day = sprintf ('%02s',$day);
                    my $berekende_startdatum_nieuw_contract =  "$year-$month-$day";
                    if ($berekende_startdatum_nieuw_contract eq $contracten->{$rij[0]}->{begin_datum}) {                        
