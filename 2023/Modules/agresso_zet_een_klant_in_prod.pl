...
package main;
     ...
     our $as400;
     our $plaats_sjablonen;
     main->load_agresso_setting('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\zet_klant_in_agresso_settings.xml'); #is nagezien
     main->load_brieven_setting('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\zet_klant_in_brieven_settings.xml'); #is nagezien
     main->load_brieven_vervang_teksten('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\zet_klant_in_brieven_vervang_tekst.xml'); #nagezien
     main->load_PdfToAgresso_instellingen('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\zet_klant_in_PdfToAgresso_settings.xml'); #nagezien
     main->load_PdfToAgressoScanning_settings('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\zet_klant_in_PdfToAgressoScanning_settings.xml'); #nagezien
     main->load_as400_settings('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\zet_klant_in_AS400_settings.xml'); #nagezien
     main->load_plaats_sjablonen('P:\OGV\ASSURCARD_PROG\assurcard_settings_xml\plaats_sjablonen_settings.xml'); #nagezien
     ...
     if (-e $macro_pdf_openoffice) {
          ...
          openofficemacro->maak_pdf_conversion;
     }
     foreach my $zkf_test (keys $main::PdfToAgresso_instellingen->{ziekenfondsen}) {
          my $user= $main::PdfToAgresso_instellingen->{ziekenfondsen}->{"$zkf_test"}->{as400_user};     	     #username as400
          my $pass=decrypt->new($main::PdfToAgresso_instellingen->{ziekenfondsen}->{"$zkf_test"}->{as400_paswoord});
          print "\n\nuser :$user\n\npas: $pass\n\n";
          my $condb =connectdb->connect_as400 ($user,$pass,'airbus');
          connectdb->disconnect($condb) if (defined $condb);
     }

     our $teksten_GKD = main->teksten_gkd();;  #voor naar het gkd te sturen
     ...
     sub new {
         ...
         $dbh_agresso = sql_toegang_agresso->setup_mssql_connectie;
         ...
         if ($rijks_register_nummer eq 'nok') {
             ...
             $rijks_register_nummer = sprintf("%011s",$rijks_register_nummer);
                ...
                if ($main::klant->{Agresso_nummer} ) {
                    ...
                    $bestaande_klant =1;
                    $gevonden = main->zoek_verzekerden ($dbconnectie,$ziekenfonds_nummer,$externnummer) ;
     ...
     sub load_brieven_vervang_teksten  {
         my ($class,$file_name) =  @_;
         print "$file_name ->";
         $brieven_vervang_teksten = XMLin("$file_name");
         print "ingelezen \n";
          #foreach my $zkf_inst (keys $agresso_instellingen->{verzekeringen}) {
          #   #my $verz_inst =$agresso_instellingen->{verzekeringen}->{$zkf_inst};
          #   foreach my $verz_inst  (sort keys $agresso_instellingen->{verzekeringen}->{$zkf_inst}) {
     ...
     sub load_plaats_sjablonen {
            my ($class,$file_name) =  @_;
            print "$file_name ->";
            $main::plaats_sjablonen = XMLin("$file_name");
            print "ingelezen\n";
     }
     ...
     sub send_via_webserv_client {
            ...
            -> proxy('http://S200WP1XXL01.mutworld.be/BusinessWorld-webservices/service.svc?ImportService/ImportV200606')
...
package FrameDialog;
     ...
     sub new {
         ...
         my $rijkregnr ='';
         my $zkf ='';
         $self = $self->SUPER::new(undef, -1, "Geef een rijksregister nummer en een ziekenfonds in:",
     ...
     sub OK {
         ...
         if ($zkf  =~ m/\d{3}/ and $rijkregnr=~ m/\d{8}/) {
             ...
             my $dbconnectie = connectdb->connect_as400 ($settings->{user_name},$settings->{password},$settings->{name_as400});
             my $min_einddatum_contract = $vandaag -20000;
             $externnummer =as400_gegevens->natreg_to_extern($dbconnectie,$rijkregnr,$settings,$min_einddatum_contract);
             $main::klant->{ExternNummer}=$externnummer;
...
package Frame;
     ...
     sub new {
          ...
          $self = $self->SUPER::new(undef, -1, "MOBICOON Productie Milestone 4",
...
package Lid_Opname_Verzekering;
     ...
     sub RijksRegister_Nummer {
         ...
         foreach my $zf  (keys $main::agresso_instellingen->{verzekeringen}) {
             ...
             my $externnummer =as400_gegevens->natreg_to_extern($dbconnectie,$rr_nr,$settings,$min_einddatum_contract);
             if ($externnummer =~ m/\d+/) {
                  my $bool = $frame->Close();
                  $lid_gevonden=1;
                ...
                $was_lid_ext_nr =as400_gegevens->natreg_to_extern($dbconnectie,$rr_nr,$settings,$min_einddatum_contract);
                if ($was_lid_ext_nr =~ m/\d+/) {
                     $was_lid_zkf =$zf_nr ;
                    }
               }

          }
         if ($lid_gevonden == 0 and $was_lid_zkf ne '' and $was_lid_ext_nr =~ m/\d+/) {
           main->new($was_lid_ext_nr,$was_lid_zkf);
          }
     }
     sub ArgV_RijksRegister_Nummer {
         ...
         foreach my $zf  (keys $main::agresso_instellingen->{verzekeringen}) {
             ...
             my $dbconnectie = connectdb->connect_as400 ($settings->{user_name},$settings->{password},$settings->{name_as400});
             my $min_einddatum_contract = $vandaag;
             my $externnummer =as400_gegevens->natreg_to_extern($dbconnectie,$rr_nr,$settings,$vandaag);
             if ($externnummer =~ m/\d+/) {
                  my $bool = $frame->Close();
                  $lid_gevonden=1;
                  main->new($externnummer,$zf_nr);
...
use DateTime;
    sub new {
        ...
        $frame->{brieven_sizer_1}->Add($frame->{brieven_panel_2}, 0, wxALIGN_BOTTOM|wxALIGN_RIGHT);
         ...
         Wx::Event::EVT_CHECKBOX($frame,$frame->{brieven_chk_3_Contract}, \&checkbox_2_Contract_clicked);
         Wx::Event::EVT_CHECKBOX($frame,$frame->{brieven_chk_4_Contract}, \&checkbox_2_Contract_clicked);
      }
...
#our $dbh_mssql;
        ...
        sub setup_mssql_connectie {
             ...
             # connect to database
             #
             $dbh_mssql = DBI->connect($dsn_mssql, $user, $passwd, $db_options) or sql_toegang_agresso->cannot_connect($user);
             return ($dbh_mssql)
            }
        sub cannot_connect {
             my( $self,$user_name ) = @_;
