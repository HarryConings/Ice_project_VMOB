...
package main;
     ...
     print "----------------------------------------------------------------------\n\n";
     our $PdfToAgressoScanning_instellingen = XMLin('D:\OGV\ASSURCARD_2023\assurcard_settings_xml\agresso_settings.xml');
     our $mode = 'TEST';
     $mode = $ARGV[0] if (defined $ARGV[0]);
     if ( $mode eq 'TEST' or $mode eq 'PROD'){}else{die}
     my $werkings_mode = $mode;
         ...
         foreach my $mail (@mails) {
             ...
             if ($antwoord eq 'ok' and defined $zkf_nr) {
                 ...
                 if ($gelukt[0] eq 'gelukt') {
                     ...
                     unlink $main::file if ($mode eq 'PROD');
                     $teller_ok +=1;
...
package webservice;  
     ...
     sub PDF_naar_Agresso {
         ...
         my $prox = $PdfToAgressoScanning_instellingen->{"Agresso_IP_$main::mode"};
         my $proxy = "http://$prox/service.svc?CustomerService/Customer";# test
     ...
     sub agresso_get_customer_info_rr_nr {
      ...
      my $prox = $PdfToAgressoScanning_instellingen->{"Agresso_IP_$mode"};
      my $proxy = "http://$prox/service.svc"; #productie    
