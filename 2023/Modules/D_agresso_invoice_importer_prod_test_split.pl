...
sub read_assurcard_invoice {
     ...
     $file = 'P:\OGV\ASSURCARD_PROG\assurcard_invoices\049.invoice.out.20230726.000416.xml';
     $file = 'P:\OGV\ASSURCARD_PROG\assurcard_invoices\322.invoice.out.20230802.002028.xml';
     my $batch = $&;
             ...
             if ($@) {
                 ...
                 foreach my $volgnr_Col (keys @{$assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Col}}) {
                     ...
                     if ($assurcard->{Invoice}->{Groups}->{Group}->{Line}->{Col}[$volgnr_Col]->{Type} eq "Type") {
                         ...
                         $Description =~ s/\s{2}//g;
                         $Description =~ s/\s{2}//g;
                         $Description =substr($Description,-70);
                       }
                     ...
                     if (!$@) {
                          ...
                          if (!$@) {
                               ...
                               $Description_warning =~ s/\s{2}//g;
                               $Description_warning =~ s/\s{2}//g;
                               $Description_warning = substr($Description_warning,-70);
...
sub maak_xml_invoice {
     my $xsd ='D:\OGV\ASSURCARD_PROG\asurcard_xsd\agresso\ABWTransaction-har.xsd';
     my $schema = XML::Compile::Schema->new($xsd);                      
     $schema->importDefinitions('D:\OGV\ASSURCARD_PROG\asurcard_xsd\agresso\ABWSchemaLib-har.xsd');#,
