...
#&read_assurcard_invoice('P:\OGV\ASSURCARD_PROG\assurcard_invoices\714.invoice.out.20210624.003407.xml');
                     ...
                     foreach my $volgnr_Line (keys @{$assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}}) {
                         ...
                         foreach my $volgnr_Col (keys @{$assurcard->{Invoice}[$volgnr_invoice]->{Groups}->{Group}->{Line}[$volgnr_Line]->{Col}}) {
                             ...
                             if (!$@) {
                               ...
                               if (!$@) {
                                    print "$volgnr_invoice  $volgnr_Line\n";
                                    if ($volgnr_invoice == 0 and $volgnr_Line==7) {
                                        print'';
                                    }
